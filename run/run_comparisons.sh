#!/bin/bash
# Exit immediately if a command exits with a non-zero status
set -e

# 1. Setup Environment
# Sourcing the LCG view provides uproot, sklearn, and a modern Python 3
LCG_VIEW="/cvmfs/sft.cern.ch/lcg/views/LCG_104/x86_64-el9-gcc11-opt/setup.sh"

if [ -f "$LCG_VIEW" ]; then
    source "$LCG_VIEW"
    echo ">>> Sourced LCG environment from $LCG_VIEW"
else
    echo "ERROR: Could not find LCG view at $LCG_VIEW. Are you on a machine with CVMFS access?"
    exit 1
fi

# 2. Default directories
ROOT_DIR="/afs/cern.ch/user/e/edweik/private/new_ad_files"
FITS_DIR="/afs/cern.ch/user/e/edweik/private/GlobalSignificanceSearch/fits"

# Parse optional command-line arguments
while [[ "$#" -gt 0 ]]; do
    case $1 in
        -r|--root-dir) ROOT_DIR="$2"; shift ;;
        -f|--fits-dir) FITS_DIR="$2"; shift ;;
        -h|--help)
            echo "Usage: $0 [options]"
            echo "Options:"
            echo "  -r, --root-dir <path>   Path to ROOT files (default: $ROOT_DIR)"
            echo "  -f, --fits-dir <path>   Path to JSON fit files (default: $FITS_DIR)"
            exit 0
            ;;
        *) echo "Unknown parameter passed: $1"; exit 1 ;;
    esac
    shift
done

echo "======================================================"
echo " Starting Background Model Comparisons"
echo "======================================================"
echo " ROOT Directory: $ROOT_DIR"
echo " FITS Directory: $FITS_DIR"
echo " Python Version: $(python3 --version)"
echo "------------------------------------------------------"

# Run the python script
python3 python/compare_all_fits.py \
    --root-dir "$ROOT_DIR" \
    --fits-dir "$FITS_DIR"

echo "======================================================"
echo " Finished."
echo "======================================================"
