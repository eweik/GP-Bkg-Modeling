...
def main():
    parser = argparse.ArgumentParser(description="Generate 9-panel grids for GP Signal Injection (Locked Kernel).")
    ...
    out_dir = "plots/advanced_injection_grid"
    os.makedirs(out_dir, exist_ok=True)

    channels = ["jj", "jb", "bb", "be", "bm", "bg", "je", "jm", "jg"]
    triggers = [f"t{i}" for i in range(1, 8)]

    print(f"\nStarting 9-Panel LOCKED Kernel Injection Grid (Length Scale Bound: {args.min_len*100:.1f}%)")
    print("-" * 60)
    ...
    for t in triggers:
        ...
        fig.suptitle(f'LOCKED GP Signal Absorption (5$\sigma$ Injection) - Trigger {t.upper()}', fontsize=20, fontweight='bold', y=0.96)
        ...
        for i, ch in enumerate(channels):
            ...
            # ==========================================
            # PLOT: ABSORPTION PANEL
            # ==========================================
            ax.plot(valid_masses, absorptions, 'bo-', lw=2, markersize=8, label='Signal Absorption')
            
            # ATLAS pass/fail threshold line
            ax.axhline(20.0, color='r', linestyle='--', label='20% Threshold')
            ...
            y_limit = max(30.0, max(absorptions) + 5) if absorptions else 30.0
        ...
        out_file = os.path.join(out_dir, f"grid_injection_{t}.png")
        fig.savefig(out_file, bbox_inches='tight', dpi=200)
        plt.close(fig)
        
        completed_grids += 1
        sys.stdout.write(f"\rGenerated {completed_grids} 9-panel injection grids... ")
        sys.stdout.flush()

    print("\n" + "-" * 60)
    print("Finished. Check the plots/advanced_injection_grid/ directory.")
