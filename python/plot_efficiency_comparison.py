...
def main():
    parser = argparse.ArgumentParser(description="Compare GP vs 5p Signal Efficiency.")
    parser.add_argument("-t", "--trigger", type=str, default="all")
    parser.add_argument("-c", "--channel", type=str, default="all")
    ...
    parser.add_argument("--toys", type=int, default=10)
    args = parser.parse_args()

    out_dir = "plots/efficiency_comparisons"
    os.makedirs(out_dir, exist_ok=True)

    all_channels = ["jj", "jb", "bb", "be", "bm", "bg", "je", "jm", "jg"]
    all_triggers = [f"t{i}" for i in range(1, 8)]

    triggers = all_triggers if args.trigger.lower() == 'all' else [args.trigger.lower()]
    channels = all_channels if args.channel.lower() == 'all' else [args.channel.lower()]
    num_iterations = 1 if args.method == 'asimov' else args.toys

    print(f"\nStarting Efficiency Sweep ({args.method.upper()})")
    print("-" * 80)
    ...
    for t in triggers:
        ...
        for ch in channels:
            ...
            test_masses = np.linspace(c_fit[0] + 0.10*mass_span, c_fit[-1] - 0.10*mass_span, 20)
            ...
            ax.errorbar(valid_masses_tev, mean_eff_5p, yerr=err_eff_5p, fmt='ro--', lw=2, capsize=4, markersize=6, label='Legacy 5-Param (Floated)')
            ax.errorbar(valid_masses_tev, mean_eff_gp, yerr=err_eff_gp, fmt='bo-', lw=2, capsize=4, markersize=6, label=f'Advanced GP (Locked L > {args.min_len*100:.0f}%)')

            ax.axhline(1.0, color='k', linestyle='--', lw=2)
            ax.axhline(0.8, color='k', linestyle=':', lw=2, label='ATLAS Minimum (80%)')
            
            title_suffix = "Asimov" if args.method == 'asimov' else f"{args.toys} Poisson Toys/Mass"
            ax.set_title(f"Efficiency Comparison: {t.upper()} | {ch.upper()}\n(5-Sigma Injection, {title_suffix})", fontsize=14, fontweight='bold')
            ax.set_xlabel(f"Injected Mass [TeV]", fontsize=13)
            ax.set_ylabel("$N_{rec} / N_{inj}$ (Efficiency)", fontsize=13)
            
            ax.set_ylim(0.1, 1.5)
            ax.legend(loc='lower right', fontsize=11, framealpha=0.9)
            ax.grid(True, alpha=0.3, linestyle='--')

            out_file = os.path.join(out_dir, f"Eff_Comp_{args.method.capitalize()}_{t}_{ch}.png")
            plt.savefig(out_file, bbox_inches='tight', dpi=300)
            plt.close()
            
            completed += 1
            sys.stdout.write(f"\rProcessed {completed} comparisons... ")
            sys.stdout.flush()

    print("\n\n" + "=" * 105)
    print(f"{'SIGNAL EFFICIENCY TOY METRICS (N_rec / N_inj)':^105}")
    print("=" * 105)
    print(f"| {'Trig':<4} | {'Chan':<4} || {'Avg Eff (5p / GP)':<20} || {'Worst Eff (5p / GP)':<22} || {'Avg RMS Spread (5p / GP)':<24} |")
    print("-" * 105)
    for row in results_table:
        ...
        print(f"| {row['trigger']:<4} | {row['channel']:<4} || {avg_eff_str:<20} || {min_eff_str:<22} || {rms_str:<24} |")
    print("=" * 105)
    print(f"Sweep Finished. Generated: {completed} | Skipped: {skipped}")
