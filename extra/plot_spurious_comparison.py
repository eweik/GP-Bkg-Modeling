...
def main():
    parser = argparse.ArgumentParser(description="Spurious Signal Toys: GP vs 5p.")
    ...
    out_dir = "plots/spurious_comparisons"
    ...
    print(f"\nStarting Spurious Signal Sweep ({args.method.upper()})")
    print("-" * 65)
    ...
    for t in triggers:
        ...
        for ch in channels:
            ...
            ax.errorbar(valid_masses_tev, mean_spur_5p, yerr=err_spur_5p, fmt='ro--', lw=2, capsize=4, markersize=6, label='Legacy 5-Param (Floated)')
            ax.errorbar(valid_masses_tev, mean_spur_gp, yerr=err_spur_gp, fmt='bo-', lw=2, capsize=4, markersize=6, label=f'Advanced GP (Locked L > {args.min_len*100:.0f}%)')

            ax.axhline(0.0, color='k', linestyle='-', lw=1)
            ax.axhline(0.5, color='k', linestyle='--', lw=2)
            ax.axhline(-0.5, color='k', linestyle='--', lw=2, label='Typical Acceptable Threshold ($\pm 0.5\sigma$)')
            ...
            ax.set_title(f"Spurious Signal Comparison: {t.upper()} | {ch.upper()}\n({title_suffix})", fontsize=14, fontweight='bold')
            ax.set_xlabel(f"Testing Mass [TeV]", fontsize=13)
            ax.set_ylabel("$S_{spur} \ / \ \sigma_{fit}$", fontsize=13)
            
            y_max = max(1.0, np.max(np.abs(mean_spur_5p)) * 1.2)
            ax.set_ylim(-y_max, y_max)
            
            ax.legend(loc='upper right', fontsize=11, framealpha=0.9)
            ax.grid(True, alpha=0.3, linestyle='--')

            out_file = os.path.join(out_dir, f"Spur_Comp_{args.method.capitalize()}_{t}_{ch}.png")
    ...
    print("\n\n" + "=" * 95)
    print(f"{'SPURIOUS SIGNAL TOY METRICS':^95}")
    print("=" * 95)
    print(f"| {'Trig':<4} | {'Chan':<4} || {'Max |S|/err (5p) @ Mass':<25} || {'Max |S|/err (GP) @ Mass':<25} || {'Avg RMS (5p / GP)':<17} |")
    print("-" * 95)
    for row in results_table:
        ...
        print(f"| {row['trigger']:<4} | {row['channel']:<4} || {m5p_str:<25} || {mgp_str:<25} || {rms_str:<17} |")
    print("=" * 95)
