...
def get_gp_fit(centers, density, density_err, min_len_scale_log=0.15, max_len_scale_log=5.0):
    ...
    # Standard zero-mean GP with length scale bounds
...
def main():
    parser = argparse.ArgumentParser(description="Generate 9-panel grids for Zero-Mean GP Spectra and Pull Distributions.")
    ...
    out_dir = "plots/zeromean_comparisons_grid"
    os.makedirs(out_dir, exist_ok=True)

    channels = ["jj", "jb", "bb", "be", "bm", "bg", "je", "jm", "jg"]
    triggers = [f"t{i}" for i in range(1, 8)]

    print(f"\nStarting 9-Panel Zero-Mean GP Generation (Length Scale Bounds: [{args.min_len}, {args.max_len}])")
    print("-" * 60)
    ...
    for t in triggers:
        ...
        # Initialize Figures for this trigger (3x3 grid = 9 masses on 1 panel/figure)
        fig_spec, axes_spec = plt.subplots(3, 3, figsize=(18, 15))
        fig_pull, axes_pull = plt.subplots(3, 3, figsize=(18, 15))
        
        fig_spec.suptitle(f'Spectral Fits (Zero-Mean GP vs Nominal) - Trigger {t.upper()}', fontsize=20, fontweight='bold', y=0.96)
        fig_pull.suptitle(f'Pull Distributions - Trigger {t.upper()}', fontsize=20, fontweight='bold', y=0.96)
        ...
        for i, ch in enumerate(channels):
            ...
            ax_spec.plot(c_fit, gp_density, 'b-', lw=2, label='Zero-Mean GP')
            ...
            if len(v_leg) > 0 and len(v_gp) > 0:
                for v, color, label in zip([v_leg, v_gp], ['red', 'blue'], [fit_name, 'Zero-Mean GP']):
                    mu, std = norm.fit(v)
