...
def main():
    parser = argparse.ArgumentParser(description="Plot Full Pearson Matrix: Raw vs Copula (GP Background)")
    ...
    for i, col in enumerate(col_names):
        ...
        data = get_channel_data_gp(base_dir, args.trigger, channel, args.cms, mass_matrix, col_names, args.min_len)
    ...
    sns.heatmap(corr_copula, ax=axes[1], cmap=cmap, vmin=vmin, vmax=vmax,
                xticklabels=col_names, yticklabels=col_names, 
                annot=True, fmt=".2f", square=True, cbar_kws={"shrink": .8})
    axes[1].set_title(f"Copula Toys Pearson $\\rho$ (Mapped to GP Null)\n{args.trigger.upper()}", fontsize=14)

    plt.suptitle("Comparison of Global Linear Correlation (Pearson) via GP", fontsize=18, fontweight='bold', y=1.02)
    fig.tight_layout(rect=[0, 0, 1, 0.96])
    
    os.makedirs(os.path.join(base_dir, "plots"), exist_ok=True)
    out_path = os.path.join(base_dir, "plots", f"full_pearson_matrix_comparison_gp_{args.trigger}.png")
    plt.savefig(out_path, dpi=300, bbox_inches='tight')
    print(f"\nSuccessfully saved GP Pearson correlation matrix plot to: {out_path}")
